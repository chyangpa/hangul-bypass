...
| **붙여넣기** | `ctypes.SendInput` + `KEYEVENTF_UNICODE` 직접 호출 — 키 release/re-press 없이 훅 간섭 회피 |
...
- **클립보드 붙여넣기** — 채팅창에서 `Ctrl+V`로 클립보드 텍스트 직접 주입
- **채팅창 상태 추적** — Enter/Esc/마우스 클릭으로 채팅 열림/닫힘 감지, 모드 자동 전환
- **모드 기억** — 채팅창을 닫았다 열어도 이전 한/영 모드 유지
- **활성 창 감지** — 게임 윈도우가 포커스일 때만 동작 (다른 앱에 영향 없음)
- **터미널 UI** — HD2 테마 배너, 현재 모드/채팅 상태 실시간 표시
- **설치 불필요** — exe 파일 1개만 다운로드하여 실행 (레지스트리 수정 없음)
- **오버레이 방식 아님** — 게임 화면 위에 덮어씌우지 않음
...
- 상태가 꼬이면 `Esc`나 마우스 좌/우 클릭으로 초기화

> - Edge 다운로드 경고: `⋯` → `유지` → 삭제 버튼 옆 드롭다운(`∨`) → `그래도 계속`
> - 실행 시 SmartScreen 경고: `추가 정보` → `실행`
...
| `Ctrl+V` | 클립보드 텍스트 붙여넣기 (채팅창 열린 상태에서만) |
...
| 마우스 훅 | [mouse](https://github.com/boppreh/mouse) |
